...
同时 ADR-0020 把视觉基准改为设计规范后，组件层按「按设计规范结构与样式」推进——设计规范的每个组件
都写着 `t("…")`。没有 i18n 基建时按规范实现的组件只能把 `t()` 换回硬编码串，等于**每引入一个组件就制造一次
反向重写**；反过来先落 i18n 则组件可以直接保留 `t()`。

2026-09-26 的前端一致性审查（`docs/09-frontend-design-checklist.md`）据此提出该决策应被推翻，
并经用户确认范围：**要做 i18n，三语 = 中文 / 英文 / 日语**。

## 决策

**引入设计规范的 registry 式 i18n，做 `en` / `zh-CN` / `ja` 三语；Provider 放 `packages/ui`。**

1. **架构按设计规范**（体量很小，纯新增、零冲突）：
...
7. **修订 ADR-0020 §4 的一句**：该条把「mermaid 缩放」列为排除域示例，与审查结论不符——
   **mermaid 不在排除域**（排除域只有移动端 / 子代理 / 终端 / Provider 用量 / 登录 / PWA）。
   本次已把 `.katex*` 与 `.mermaid-block*` / `.mermaid-zoom-*` 规则补进 `styles/web-ui.css`。
...
-按设计规范组件时可直接保留 `t()`，不再产生「引入又改写」的反向工作
- 一门新语言 = 新增一个 `messages/<id>.ts` + registry 加一行，成本线性
- 三语 key/占位符一致性有测试锁；`en` 与 zh-CN 的 key 集合与设计规范同源，未来对齐上游可 diff

**负面 / 已知风险**

- `ja` 无上游可 diff：后续设计规范新增 key 时，`en`/`zh-CN` 可追平、`ja` 必须补译（漏译会被一致性测试拦下）
