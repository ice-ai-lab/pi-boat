...
另有一个被否决的选项值得记录：pi-web 的做法是"全量会话列表 + 客户端推导项目清单"。推导要求列表**全量且不分页**，因此**分页与客户端推导互斥**——一旦分页，项目清单就残缺且随活跃会话漂移。
...
- `?force=1`：跳过缓存并清空项目解析缓存（对齐 pi-web 语义；此前该参数收了但被忽略）
...
| 纯客户端推导项目清单（pi-web 路线） | ❌ 放弃 | 与分页互斥；且推导要在客户端重写归一逻辑（含 git），两面重复 |
...
| 缓存用 TTL（pi-web：列表 30s + git 60s） | ❌ 放弃 | TTL 到期必须全量重扫（10³ 会话时每次卡 ~2.5 s），且窗口内会返回陈旧结果；指纹键两者都避免。补充：pi-web 的 `sessionListVersion` 实际是**缓存 generation 计数器**（`__piSessionListGeneration`，仅在 `force=1` 清缓存时 +1），因此它同样不反映跨进程写入——我们的指纹是对该字段语义的实质改进 |
