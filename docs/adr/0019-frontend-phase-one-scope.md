# ADR-0019：前端一期范围与复用策略——按设计规范页面功能（六项执行决策）
...
一期后端已全部交付（`docs/07` §1），前端（client / ui / web）开工前需要把「按设计规范页面功能」落到
可执行的排期。规划本体在 `docs/08`：以设计规范源码快照（npm 0.9.1，~28 700 行前端）为基准的功能盘点、
复用三分法、三包落位与 F0–F5 批次。其中六个分叉**跨越多份文档、且容易被后续会话重新提出**，
本文把它们固化为决策记录并写明推翻代价。六项均由用户于 2026-09-27 确认。

总纲（随本文一并生效，细则见 `docs/08` §1/§2/§5）：**一期范围 = 设计规范页面功能全集 − ADR-0014 排除域**
（差异清单 15 项，前端不得自行补回）；**复用三分法** = A 类纯逻辑直接落地（测试先行）、
B 类组件落地行为与结构但视觉按本仓原型 v3 重写（**决策 6 已被 ADR-0020 推翻**：视觉改为按设计规范）、C 类排除域与服务端域不落地。
...
（设计规范为 en / zh-CN / zh-TW 三语 + registry 方案；本仓文档不做双语维护——AGENTS.md。）
...
（localStorage）实现，与设计规范同法。core 落盘（`setDefaultModelAndProvider()`）**不做**；
G2-11 保持非阻塞缺口（`docs/07` §2/§3.4/§4/§9 已同步标注）。

### 5. URL 形态：单路由 `/` + `?s=<sessionId>` ✅

React Router v7 库模式单路由，当前会话以 `?s=` 持久（按设计规范的 searchParams 模式，
与 tab-session / workspace-memory 配合）；设置与顶部面板均为浮层，不占路由。

### 6. 视觉基准：功能按设计规范、视觉走本仓原型 v3 ✅（**已被 ADR-0020 取代**）

「一样的页面功能」不含像素对齐——设计规范的手写 CSS 变量体系与本仓 Tailwind v4 + shadcn 不兼容，
...
| # | 被否决的备选 | 理由 |
|---|---|---|
| 1 | 引设计规范的 i18n registry 全量落地三语 | 三语是设计规范的增量负担；本仓文档已定不做双语，UI 单语与之一致 |
| 2 | 深色维持「M2 再说」 | 原型 dark token 已完整、设计规范三态现成，推迟无收益反留悬案 |
...
| 6 | 视觉也按设计规范 | 须推翻原型 v3 与 docs/06 全部 token 映射，成本最大、收益为零 |
...
- 深色进 F5，若原型 dark token 有对比度缺陷会在验收后期暴露（缓解：token 与原型一致，F5 专项验证）
- ui 例外四处是对「不取数」边界的松动，存在例外扩散风险（防线：逐个登记 + 第二用途论证）
- G2-11 前端绕过意味着清浏览器存储 / 换浏览器丢偏好（设计规范同款行为，可接受）
