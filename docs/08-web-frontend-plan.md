...
| 批次 | 内容 | 主要产出 | 粗估 |
|---|---|---|---|
| **F0 骨架** | `apps/web` 建包（Vite+RR7+Query+Tailwind v4+React Compiler）；client 双入口工程化（`./react` 子导出）；ui `theme.css` + primitives 首批（Button/IconButton/Popover/Textarea/Toast/ScrollArea）；`MobileGate` 与 proxy；Playwright 冒烟 | 空壳三栏页 + `/api/health` 打通 | 0.5 周 |
| **F1 对话 MVP** | client：http / agent+sessions 端点 / stream 四件套（fold/rebuild/group-trail/agent-source）/ `use-agent-session`；ui：primitives 补齐 + chat 全册（MessageList/AssistantTurn/Composer/ToolTag/ThinkingRow/ProcessGroup/StatsPills/SystemPromptPanel 骨架）；web：EmptyState cwd 输入（docs/06 §11.3-1）+ 冷会话 resume（ADR-0013a） | **M1 验收线：浏览器完成一轮带工具调用的编程任务** | 1.5–2 周 |
