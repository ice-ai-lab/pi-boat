...
`SessionListItem`（`.session-item` + `.ops` hover 操作 + 运行中标记）、`FileTree`（`.tnode` /
...
- **用户手动展开过的行/组不得被自动收起覆盖** → `TrailItem` 需带 `userToggled` 标记（docs/05 §6.1/§6.5）
...
Tailwind CSS v4（`@theme inline`）、shadcn/ui 落位本包 `primitives/`、React Compiler、
Lucide、`cva` + `clsx` + `tailwind-merge`（`cn()`）、markdown 走 `react-markdown` + `remark-gfm`
+ `shiki` + `rehype-sanitize`（禁裸 `dangerouslySetInnerHTML`）。**ui 不引 TanStack Query**（§1 边界 2）。
