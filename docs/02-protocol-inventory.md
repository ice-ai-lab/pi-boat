...
| 5 | 系统提示词「版本 r42」 | `AgentState.systemPrompt` 已在 M1 契约内（**无需协议改动**）；但无版本号字段 | ✅ **已定（2026-09-22）：展示，删掉「版本 r42」**（无数据来源）。面板规格已定：只渲染原始文本 + 两态（尚未加载 / 加载中），**不显示版本号也不做 token 估算**，且**不声称「实际下发」**（本仓不做精确覆写，ADR-0015）。详见 `docs/06` §11.2 行 5 |
