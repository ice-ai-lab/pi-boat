...
**下一步**：前端一期按 `docs/08`（已定案，ADR-0019）的 F0–F5 批次推进——✅ F0 骨架（2026-09-27）；✅ F1 对话 MVP（2026-09-27：**M1 验收线达成**）；✅ F2 会话与项目（2026-09-27）；✅ F3 文件域（2026-09-27）；✅ F4 设置中心（2026-09-27）；✅ **F5 对话增强与高级面板（2026-09-27：一期 F0–F5 全部完成）**——斜杠命令/输入历史/排队条/压缩与自动命名/导出/统计与分支面板/系统提示词与工具面板/minimap/本轮改动文件/扩展 UI（状态栏·widgets·阻塞对话框）/深色主题三态/快捷键/提示音与页内通知/shiki 高亮。
